...
public class MyServlet extends JsonServlet{
    ...
    // он не создает, а получает от фабрики
    // это важно - при смене реализации интерфейс останется тот же
    // делаем это в 1 месте в фабрике. а все классы,
    // которые используем подтянут ту версию, которая прописана в этой фабрике
    private UserService userService = UberFactory.getINSTANCE().getUserService();

    // создание анонимного класса
    Map<String, User> users = new HashMap<String, User> (){{
        put ("1", new User().setName("Vasia").setRole("admin"));
        put("2", new User().setName("Petia").setRole("user"));
    }};
